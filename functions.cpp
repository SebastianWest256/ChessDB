...
float cx(float i, camera c) {
    return i - c.position.x;
}
float cy(float i, camera c) {
    return i - c.position.y;
}
...
bool areColorsEqual(const SDL_Color& c1, const SDL_Color& c2) {
    return c1.r == c2.r && c1.g == c2.g && c1.b == c2.b && c1.a == c2.a;
}
std::vector<std::vector<piece>> fenToBoard(const std::string& fen) {
    const SDL_Color
    ...
    for (char ch : ranks) {
        ...
        if (isdigit(ch)) {
            ...
            SDL_Color color = isupper(ch) ? pwhite : pblack;
            switch (tolower(ch)) {
            case 'p': board[col][row].type = 1; break;
            case 'r': board[col][row].type = 2; break;
            case 'n': board[col][row].type = 3; break;
            case 'b': board[col][row].type = 4; break;
            case 'k': board[col][row].type = 5; break;
            case 'q': board[col][row].type = 6; break;
            }
...
std::string boardToFen(const std::vector<std::vector<piece>>& board) {
    ...
    for (int row = 0; row < 8; row++) {
        ...
        for (int col = 0; col < 8; col++) {
            if (board[col][row].type == 0) {
                ...
                case 1: fen << (areColorsEqual(board[col][row].color, pwhite) ? 'P' : 'p'); break;
                case 2: fen << (areColorsEqual(board[col][row].color, pwhite) ? 'R' : 'r'); break;
                case 3: fen << (areColorsEqual(board[col][row].color, pwhite) ? 'N' : 'n'); break;
                case 4: fen << (areColorsEqual(board[col][row].color, pwhite) ? 'B' : 'b'); break;
                case 5: fen << (areColorsEqual(board[col][row].color, pwhite) ? 'K' : 'k'); break;
                case 6: fen << (areColorsEqual(board[col][row].color, pwhite) ? 'Q' : 'q'); break;
                }
            }
        }
